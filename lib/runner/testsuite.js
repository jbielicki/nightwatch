...
TestSuite.prototype.run = function() {
  ...
  if (this.module.isDisabled()) {
    ...
    const skippedSteps = this.module.skipSteps().filter((el) =>
    el !== 'client' && el !== 'before' && el !== 'after' && el !== '@skipSteps' && Object.keys(this.module['@module']).includes(el));
    this.setCurrentTest();
    this.globalBeforeEach()
      ...
      .then(function() {
        self.testResults.steps = [...skippedSteps, ...self.testResults.steps];
        self.deferred.resolve(self.testResults);
      })
      .catch(function(e) {
